...
def make_dataset(
        ...
        for split in ['train', 'test']:
            ...
            if X_cat is not None:
                if concat:
                    ...
                    print("y is concatenated to index 0 of X_cat")
                X_cat[split] = X_cat_t  
            if y is not None:
                y[split] = y_t
    else:
        ...
        for split in ['train', 'test']:
            ...
            if X_num is not None:
                if concat:
                    ...
                    print("y is concatenated to index 0 of X_num")
                X_num[split] = X_num_t
            if X_cat is not None:
                X_cat[split] = X_cat_t
            if y is not None:
    ...
    cat_col_names = [info.get('column_names')[i] for i in info.get('cat_col_idx')]
    print("Categorical Column Names:", cat_col_names)
    
    print("Categorical Column Unique Values:")
    for i in range(X_cat['train'].shape[1]):
        print(f"Category {i}", np.unique(X_cat['train'][:, i]))
