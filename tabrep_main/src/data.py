...
def cat_encode(
    ...
    if encoding is None:
        ...
        # Random instead Lexical
        # print("x before", X)
        # rng = np.random.default_rng(seed)
        # for part in X.keys():  # Iterate over train, test, etc.
        #     for col in range(X[part].shape[1]):  # Iterate over each column
        #         unique_values = np.unique(X[part][:, col])  # Find unique encoded values
        #         shuffled_values = rng.permutation(unique_values)  # Generate a random permutation
        #         value_map = dict(zip(unique_values, shuffled_values))  # Create a mapping
        #         X[part][:, col] = np.vectorize(value_map.get)(X[part][:, col])  # Apply the mapping
        # print("x after", X)
        # Random instead Lexical
...
def transform_dataset(
    ...
    if dataset.X_num['train'].shape[1] > 0 or dataset.X_num['test'].shape[1] > 0:
    ...
    if dataset.X_num['train'].shape[1] > 0 or dataset.X_num['test'].shape[1] and transformations.normalization is not None:
...
def prepare_fast_dataloader_tensor(
    x_num: torch.Tensor,
    x_cat: torch.Tensor,
    batch_size: int,
    shuffle: bool = True
    ...
    # Use FastTensorDataLoader to handle batching
    dataloader = FastTensorDataLoader(X, batch_size=batch_size, shuffle=shuffle)
