...
import numpy as np
from ablations.learnedddpm.models.utils import *
...
class GaussianMultinomialDiffusion(torch.nn.Module):
    def __init__(
        ...
        self.embeddings = nn.ModuleList(
            nn.Embedding(num_embeddings=num_cat, embedding_dim=self.ebm_dim if num_cat > 2 else 1)
    ...
    def emb_2_cat(self, x_emb):
        ...
        for item in self.categories:
            cat_idxs.append(1 if item <= 2 else self.ebm_dim)
        cat_idxs = torch.tensor(cat_idxs)[:-1].cumsum(0)

        x_cat = []
        for idx, idx_b in enumerate(cat_idxs):
            length = 1 if self.categories[idx] <= 2 else self.ebm_dim
            idx_e = idx_b + length
