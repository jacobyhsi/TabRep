...
import numpy as np
from ablations.learnedddpm_2d.models.utils import *
...
class GaussianMultinomialDiffusion(torch.nn.Module):
    def __init__(
        ...
        # for idx, num_cat in enumerate(categories):
        #     dim = 1 if num_cat <= 2 else self.ebm_dim
        #     emb = torch.randn(num_cat, dim)
        #     emb_min = emb.min(0).values.unsqueeze(0)
        #     emb_max = emb.max(0).values.unsqueeze(0)
        #     emb = (emb - emb_min) / (emb_max - emb_min) * 2 - 1
        #     self.register_buffer(f'embedding_{idx}', emb)

        self.embeddings = nn.ModuleList(
            nn.Embedding(num_embeddings=num_cat, embedding_dim=self.emb_dim if num_cat > 2 else 1)
    ...
    # Dictionary encoding part
    def look_up_dictionary(self, x_cat):
        x_cat_emb = [
            self.embeddings[idx](x_cat[:, idx].long())
            for idx in range(len(self.categories))
        ]
        return torch.cat(x_cat_emb, dim=-1)
    
    def emb_2_cat(self, x_emb):
        cat_idxs = [0]
        for item in self.categories:
            cat_idxs.append(1 if item <= 2 else self.emb_dim)
        cat_idxs = torch.tensor(cat_idxs)[:-1].cumsum(0)

        x_cat = []
        for idx, idx_b in enumerate(cat_idxs):
            length = 1 if self.categories[idx] <= 2 else self.emb_dim
            idx_e = idx_b + length

            emb = self.embeddings[idx].weight.unsqueeze(0).expand(x_emb.shape[0], -1, -1)
            dist = torch.cdist(x_emb[:, idx_b:idx_e].unsqueeze(1), emb)
            dist = dist.squeeze(1)
            x_cat.append(dist.argmin(-1))
        return torch.stack(x_cat, dim=-1)
    # Gaussian part
    ...
    def mixed_loss(self, x):
        ...
        dic_cat = self.look_up_dictionary(x_cat).detach()
        x_raw = torch.cat([x_num, dic_cat], dim=1)
        noise = torch.randn_like(x_raw)
        x_in = self.gaussian_q_sample(x_raw, t, noise=noise)

        # print("x_ohe:" + str(x_ohe.shape))
        ...
        # print(x_in)
        # print("model_out:" + str(model_out))

        loss_multi = torch.zeros((1,)).float()
        loss_gauss = torch.zeros((1,)).float()

        loss_gauss = self._gaussian_loss(model_out, x_raw, x_in, t, noise)
    ...
    def sample(self, num_samples):
        b = num_samples
        device = self.log_alpha.device
        z_norm = torch.randn(b, self.d_in, dtype=torch.float32, device=device)
        ...
        for i in reversed(range(0, self.num_timesteps)):
            ...
            z_norm = z_norm.type(torch.float32)

        z_num = z_norm[:, :self.num_numerical_features]
        z_dic = z_norm[:, self.num_numerical_features:]

        z_cat = self.emb_2_cat(z_dic)
