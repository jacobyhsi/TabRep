...
import numpy as np
from ablations.oheddpm.models.utils import *
...
class GaussianMultinomialDiffusion(torch.nn.Module):
    ...
    def mixed_loss(self, x):
        ...
        log_x_cat = index_to_log_onehot(x_cat.long(), self.num_classes)
        x_ohe = torch.cat([x_num, log_x_cat], dim=1)
        noise = torch.randn_like(x_ohe)
        x_in = self.gaussian_q_sample(x_ohe, t, noise=noise)
        ...
        # print(x_in[0])
        # print("model_out:" + str(model_out.shape))

        loss_multi = torch.zeros((1,)).float()
        loss_gauss = torch.zeros((1,)).float()

        loss_gauss = self._gaussian_loss(model_out, x_ohe, x_in, t, noise)
    ...
    def sample(self, num_samples):
        b = num_samples
        device = self.log_alpha.device
        z_norm = torch.randn((b, self.num_numerical_features + len(self.num_classes_expanded)), dtype=torch.float32, device=device)
        ...
        z_ohe_log = z_norm[:, self.num_numerical_features:]

        z_ohe = torch.exp(z_ohe_log).round()
        z_cat = ohe_to_categories(z_ohe, self.num_classes)
