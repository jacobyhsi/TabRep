...
from ablations.tabflow.models.modules import MLPDiffusion
from ablations.tabflow.models.mixed_flow import ContinuousDiscreteFlow
...
def sample(
    ...
    if num_samples > 500000:
        ...
        for i in range(5):
            print("batch", i)
            batch_samples = cfm.mixed_sample(batch_size, N=50, device=device, use_tqdm=True)
            final_data.append(batch_samples)

        syn_data = np.concatenate(final_data, axis=0)
    else:
        syn_data = cfm.mixed_sample(num_samples, N=50, device=device, use_tqdm=True)
