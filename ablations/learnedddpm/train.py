...
from ablations.learnedddpm.models.modules import MLPDiffusion
from ablations.learnedddpm.models.gaussian_multinomial_distribution import GaussianMultinomialDiffusion
    ...
    def run_loop(self):
        ...
        while step < self.steps:
            ...
            if (step + 1) % self.log_every == 0:
                ...
                # if step % 500 == 0:
                #     cur_emb = self.diffusion.embeddings[1].weight.detach().cpu()
                #     print(cur_emb) 
                #     plt.scatter(np.arange(0,len(cur_emb)), cur_emb, alpha=0.7, label=f'step={step}', s=10)
                #     plt.legend()
                #     plt.savefig("emb_viz_learnedddpm.png")
