...
import numpy as np
from ablations.dicddpm.models.utils import *
...
class GaussianMultinomialDiffusion(torch.nn.Module):
    def __init__(
        ...
        for idx, num_cat in enumerate(categories):
            emb =  torch.linspace(-1, 1, num_cat)
            emb = emb.reshape(-1, 1)
            self.register_buffer(f'embedding_{idx}', emb)
    ...
    def look_up_dictionary(self, x_cat):
        ...
        for idx, _ in enumerate(self.categories):
            emb = F.embedding(x_cat[:, idx].long(), getattr(self, f'embedding_{idx}'))
            x_cat_emb.append(emb)
    ...
    def emb_2_cat(self, x_emb):
        ...
        for idx, idx_b in enumerate(cat_idxs):
            ...
            emb = getattr(self, f'embedding_{idx}')
            emb = emb.unsqueeze(0).expand(x_emb.shape[0], -1, -1)
    ...
    def mixed_loss(self, x):
        ...
        dic_cat = self.look_up_dictionary(x_cat)
